...
export const FamilyAdminModal: React.FC<FamilyAdminModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-xs flex items-center justify-center p-[10px]">
      <div className="bg-white rounded-2xl max-w-xl w-full shadow-2xl overflow-hidden border border-slate-200">
        {/* Modal Header - Clean White Header - 10px padding */}
        <div className="bg-white p-[10px] text-slate-900 flex items-center justify-between border-b border-slate-200">
          <div className="flex items-center gap-[10px]">
            <div className="p-[8px] bg-[#f37021] rounded-lg">
          ...
          <button
            onClick={onClose}
            className="p-[6px] rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>

        <div className="p-[10px] max-h-[82vh] overflow-y-auto space-y-[10px]">
          {/* Notifications - 10px padding & gaps */}
          {successMsg && (
            <div className="bg-emerald-50 border border-emerald-200 p-[10px] rounded-xl flex items-center justify-between text-xs font-bold text-emerald-800">
              <div className="flex items-center gap-[10px]">
          ...
          {errorMsg && (
            <div className="bg-red-50 border border-red-200 p-[10px] rounded-xl flex items-center justify-between text-xs font-bold text-red-800">
              <span>{errorMsg}</span>
          ...
          {/* SECTION 1: Google Login for Admin - 10px padding & gaps */}
          <div className="bg-slate-50 rounded-xl p-[10px] border border-slate-200 space-y-[10px]">
            <h3 className="text-xs font-black uppercase tracking-wider text-[#191970] flex items-center gap-[10px]">
              <UserIcon className="w-4 h-4 text-[#f37021]" />
              <span>Stato Account Amministratore</span>
            </h3>

            {isAdminAuthenticated ? (
              <div className="flex items-center justify-between bg-white p-[10px] rounded-lg border border-slate-200">
                <div className="flex items-center gap-[10px]">
                ...
                <button
                  ...
                  className="p-[8px] text-red-600 hover:bg-red-50 rounded-lg text-xs font-bold flex items-center gap-[6px] border border-red-200 transition-colors"
              ...
              <div className="space-y-[10px]">
                ...
                <button
                  ...
                  className="w-full p-[10px] bg-white hover:bg-slate-50 border border-slate-300 text-slate-800 font-bold text-xs rounded-xl shadow-xs transition-all flex items-center justify-center gap-[10px]"
                >
                ...
                {/* Fallback Form for iFrame environment - 10px gaps */}
                <form onSubmit={handleSimulatedAdminLogin} className="pt-[10px] border-t border-slate-200">
                  <p className="text-[11px] text-slate-500 mb-[6px]">
                  ...
                  <div className="flex gap-[10px]">
                    <input
                      ...
                      className="flex-1 p-[10px] text-xs bg-white text-slate-900 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#f37021]"
                    />
                    <button
                      type="submit"
                      disabled={!simulatedAdminEmail.trim()}
                      className="px-[10px] py-[10px] bg-[#191970] text-white text-xs font-bold rounded-lg hover:bg-[#0d0d40] transition-colors disabled:opacity-50"
                    >
                      Entra Admin
                    </button>
                  </div>
          ...
          {/* SECTION 2: Members and Roles Management - 10px gaps & paddings */}
          {loading ? (
            <div className="text-center py-[20px]">
              <div className="w-8 h-8 border-3 border-[#f37021] border-t-transparent rounded-full animate-spin mx-auto mb-[10px]" />
              <p className="text-xs font-medium text-slate-500">
                Caricamento profili familiari...
              </p>
            </div>
          ) : (
            <form onSubmit={handleSaveProfiles} className="space-y-[10px]">
              {/* Family Members List Card - 10px padding & gaps */}
              <div className="bg-slate-50 border border-slate-200 p-[10px] rounded-xl space-y-[10px]">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="px-[10px] py-[8px] bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-lg flex items-center gap-[6px] shadow-xs transition-colors"
                  >
                ...
                <div className="space-y-[10px]">
                  {members.map((member, idx) => (
                    <div
                      key={member.id || idx}
                      className="bg-white border border-slate-200 p-[10px] rounded-xl flex flex-col gap-[10px] shadow-xs relative"
                    >
                      <div className="flex items-center justify-between border-b border-slate-100 pb-[6px]">
                        ...
                        {members.length > 1 && (
                          <button
                            ...
                            className="p-[6px] text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors border border-transparent hover:border-red-200"
                      ...
                      {/* Fields grid: Nome, Ruolo, Email, Password - 10px gap */}
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-[10px]">
                        <div>
                          <label className="text-[10px] font-bold text-slate-600 block mb-[4px]">
                            Nome Membro
                          </label>
                          <input
                            type="text"
                            ...
                            className="w-full p-[10px] text-xs bg-white text-slate-900 border border-slate-300 rounded-lg font-bold focus:ring-2 focus:ring-[#f37021] focus:outline-none"
                          />
                        </div>
                        ...
                        <div>
                          <label className="text-[10px] font-bold text-slate-600 block mb-[4px] flex items-center gap-[4px]">
                          ...
                          <input
                            ...
                            className="w-full p-[10px] text-xs bg-white text-slate-900 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#f37021] focus:outline-none"
                          />
                        </div>
              ...
              {/* Save Button - 10px padding */}
              <button
                type="submit"
                disabled={saving}
                className="w-full p-[10px] bg-white hover:bg-orange-50/60 border-2 border-[#f37021] text-[#f37021] font-extrabold text-xs rounded-xl shadow-xs transition-all flex items-center justify-center gap-[8px] disabled:opacity-50 cursor-pointer"
              >
